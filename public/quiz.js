...
async function loadQuestion(dir) {
  ...
  document.getElementById("options").style.display = (data.mode === "mcq") ? "block" : "none";
  document.getElementById("answer").style.display = (data.mode === "mcq") ? "none" : "block";

  if (data.mode === "mcq")
  {
    const res = await fetch("/api/getMC", {
    credentials: "same-origin"
    });
    const sus = await res.json();
    console.log(sus);
    document.getElementById("0").value = sus[0];
    document.getElementById("option0").innerText = sus[0];
    document.getElementById("1").value = sus[1];
    document.getElementById("option1").innerText = sus[1];
    document.getElementById("2").value = sus[2];
    document.getElementById("option2").innerText = sus[2];
    document.getElementById("3").value = sus[3];
    document.getElementById("option3").innerText = sus[3];
    ...
    document.getElementById("cero").style.display = (sus[0]) ? "block" : "none";
    document.getElementById("uno").style.display = (sus[1]) ? "block" : "none";
    document.getElementById("dos").style.display = (sus[2]) ? "block" : "none";
    document.getElementById("tres").style.display = (sus[3]) ? "block" : "none";
  }
  ...
  if (speakingEnabled)
  {
        ...
        {
            speak(formatForSpeech(data.question), true, data.mode);
        }
        else
        {
  ...
  (flashEnabled || data.mode === "mcq") ? "none" : "block";
  ...
  (data.timeLimit || data.mode === "speedrun") ? "block" : "none";

  // 🔥 start per-question timer
  if (!speakingEnabled) {
  const limit = Number(data.timeLimit);

        if (!Number.isNaN(limit) && data.timeLimit) {
            startTimer(limit);
        }
        else if (data.mode === "speedrun") {
            startTimer(10);
        }
  }
  if (flashEnabled)
